    ...
    <Section id="kodel-evolvia" tone="light">
      <Container>
        ...
        <Reveal className="mt-[clamp(40px,6vw,72px)]">
          ...
          {/* Full-bleed horizontal scroll on mobile. The inset padding lives on
              the edge CELLS (not the scroll container) so the sticky criterion
              column covers cleanly from the screen edge — nothing bleeds past. */}
            ...
            <table className="w-full min-w-[660px] table-fixed border-collapse text-left lg:min-w-0">
              ...
              <thead>
                <tr>
                  ...
                  <th className={cn(cell, "border-b border-border pr-6 text-[0.72rem] font-medium uppercase tracking-[0.08em] lg:pr-5", loser)}>
                    {compareColumns[1]}
                  </th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={row.criterion}>
                      <th
                        scope="row"
                        className={cn(
                          "sticky left-0 z-10 bg-bg py-5 pl-6 pr-4 text-left align-middle text-[0.9rem] font-medium text-text sm:text-[0.92rem] lg:pl-0",
                      ...
                      <td className={cn(cell, loser, "pr-6 lg:pr-5", !isLast && "border-b border-border")}>
                        <span className="mr-1.5 text-text-muted/60">—</span>
                        {row.values[1]}
                      </td>
                    </tr>
